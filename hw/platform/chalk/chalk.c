...
void platform_init_late()
{
    ...
    DRV_LOG("chalk", APP_LOG_LEVEL_INFO, "c     : %ld", SystemCoreClock);
    DRV_LOG("chalk", APP_LOG_LEVEL_INFO, "SYSCLK: %ld", RCC_Clocks.SYSCLK_Frequency);
    DRV_LOG("chalk", APP_LOG_LEVEL_INFO, "CFGR  : %ld", RCC->PLLCFGR);
    DRV_LOG("chalk", APP_LOG_LEVEL_INFO, "Relocating NVIC to 0x08004000");
}
