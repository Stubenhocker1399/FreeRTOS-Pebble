...
static void implementation_record_update(Animation *animation,
    ...
    if(s_skip_value > 0) {
        s_curr_disk_pos = GPoint(LERP(s_curr_disk_pos.x, RECORD_CENTER_X - RECORD_SWOOSH_DISTANCE), RECORD_CENTER_Y);
    } else {
        s_curr_disk_pos = GPoint(LERP(s_curr_disk_pos.x, RECORD_CENTER_X + RECORD_SWOOSH_DISTANCE), RECORD_CENTER_Y);
    }
    s_next_disk_pos =     GPoint(LERP(s_next_disk_pos.x, RECORD_CENTER_X),                          RECORD_CENTER_Y);
    layer_mark_dirty(s_main_layer);
}
...
static void implementation_arm_update(Animation *animation,
                                      const AnimationProgress distance_normalized) {
    s_arm_angle = LERP(s_animation_start_arm_angle, s_animation_end_arm_angle);
    layer_mark_dirty(s_main_layer);
}
